...
void run_io_uring_exec::operator()(const program_options& options, counters& stats) {
  ...
  for (int i = 0; i < options.nthreads; ++i) {
    ...
    threads.emplace_back(
      run_io_uring_exec_main, i, std::ref(options), files, n_bytes_per_thread, std::ref(stats));
  }
